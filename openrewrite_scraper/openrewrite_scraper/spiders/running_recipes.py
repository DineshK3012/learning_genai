...
class OpenRewriteRunningSpider(scrapy.Spider):
    name = "running_recipes"
    allowed_domains = ["docs.openrewrite.org"]
    start_urls = ["https://docs.openrewrite.org/running-recipes"]
    ...
    def parse(self, response):
        for href in response.css("a::attr(href)").getall():
            ...
            if (
                full_url.startswith("https://docs.openrewrite.org/running-recipes")
                and full_url not in self.visited
                and not full_url.endswith("/toc")
            ):
    ...
    def parse_page(self, response):
        ...
        for el in elements:
            tag = el.root.tag  # ✅ FIXED: use .tag instead of .name
            if tag in ['pre', 'code']:
    ...
    def should_skip_title(self, title):
        ...
        return any(kw in title for kw in ["edit this page", "contributors", "next", "previous", "license"])
